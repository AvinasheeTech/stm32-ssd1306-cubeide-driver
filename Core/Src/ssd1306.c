...
extern TIM_HandleTypeDef htim2; // Timer Handle :CubeIDE platform
 ...
 * @retval None.
 * @note   Updated according to CubeIDE platform
*/
...
void gpio_init(void){
  ...
  //Initialize non-SPI GPIOs (clock for port already enabled in MX_GPIO_Init)
  GPIO_InitTypeDef GPIO_InitStruct = {0};
  GPIO_InitStruct.Pin = rstPin|dcPin|csPin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
  HAL_GPIO_WritePin(GPIOA, rstPin|dcPin|csPin, LOW);
}
